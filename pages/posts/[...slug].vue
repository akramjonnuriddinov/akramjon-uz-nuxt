...
useHead({
  title: doc.value?.title ? `${doc.value.title} | Akramjon Nuriddinov` : 'Post',
...
<template>
  <div class="section-container py-32">
    <div class="max-w-3xl mx-auto">
      <NuxtLink to="/posts" class="inline-flex items-center gap-2 text-sm text-muted hover:text-primary transition-colors mb-10 group">
      ...
      <ContentDoc>
        <template #default="{ doc }">
          <article>
            <header class="mb-12">
              <div class="flex items-center gap-3 text-sm font-mono mb-4">
                <time class="text-primary/70">{{ doc.date || doc.published }}</time>
                <template v-if="doc.tags">
                  <span class="w-1 h-1 bg-muted/40 rounded-full"></span>
                  <span class="tag-primary">{{ doc.tags }}</span>
                </template>
              </div>
